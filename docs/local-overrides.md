...
The shared APM-managed Firecrawl MCP server sources `~/.zshrc.local` before launching, so keep its API key there instead of tracked agent configuration:

```bash
export FIRECRAWL_API_KEY=your-api-key
```

The shared Playwright MCP server uses Aside at `/Applications/Aside.app/Contents/MacOS/Aside`.
