...
### Zsh Configuration
- **Prompt**: Uses Kaku's generated Starship prompt when Kaku shell integration is available, otherwise falls back to [Spaceship](https://github.com/spaceship-prompt/spaceship-prompt)
- **Plugins**:
...
By default, `.zshrc` falls back to Spaceship for plain Oh My Zsh setups, but disables the Oh My Zsh theme when Kaku's generated shell integration and Starship are available so only one prompt system is active at a time. To change the fallback behavior, edit the `ZSH_THEME` logic in `.zshrc`:
```bash
