...
2. `brew-packages` - install common `Brewfile` packages, including `mise`, `chezmoi`, Hermes Agent CLI bootstrap, the Paseo desktop app and bundled CLI cask, and the Claude Code CLI cask, and perform brew-owned post-install steps
...
9. `sync-apm` - synchronize shared global skills and MCP servers through APM
