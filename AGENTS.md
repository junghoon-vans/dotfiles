...
- **Orca**: The Brewfile installs the `stablyai/orca/orca` cask. `home/dot_orca/keybindings.json` is chezmoi-managed and applied to `~/.orca/keybindings.json`. Do not track `~/.orca/agent-hooks/` — Orca regenerates those agent integration scripts itself (with machine-specific paths and runtime tokens) whenever it detects an installed coding agent like Claude Code or Codex.
