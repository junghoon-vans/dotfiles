...
- App config source lives under `home/dot_config/`: GitHub CLI preferences in `gh/config.yml`, Karabiner remapping in `karabiner/karabiner.json`, Neovim config in `nvim/`, OpenCode/OpenAgent config in `opencode/` (with the oh-my-openagent routing config in `home/dot_omo/omo.jsonc`), and Zed settings in `zed/settings.json`.
...
- OpenCode config uses the public config schema; oh-my-openagent model routing lives in the unified OMO config at `home/dot_omo/omo.jsonc` (applied to `~/.omo/omo.jsonc`). Runtime-backed LSP commands launch through `mise exec <tool@version> -- ...` without hard-coded checkout paths.
...
- **OpenCode**: `./setup.sh opencode` installs `opencode-ai`, OmniRoute, and `oh-my-openagent` globally through Bun, bootstraps oh-my-openagent with OpenAI-only model routing, and installs `opencode-status-hud`. The managed OpenCode and Zed configs expose OmniRoute's OpenAI-compatible provider and local stdio MCP server; set `OMNIROUTE_API_KEY` when the gateway requires API-key authentication. Requires Bun runtime.
- **OmniRoute persistence**: `./setup.sh opencode` loads `home/Library/LaunchAgents/com.dotfiles.omniroute.plist` (label `com.dotfiles.omniroute`) via `launchctl bootstrap`/`enable` so `omniroute serve` stays up across logins and restarts on crash (`RunAtLoad`/`KeepAlive`), matching the existing `com.dotfiles.weekly-disk-maintenance` LaunchAgent pattern. The unified OMO config at `home/dot_omo/omo.jsonc` pins free-tier routing to `omniroute/opencode-zen/deepseek-v4-flash-free` instead of `omniroute/auto/*` (which rotated among unreliable free upstreams), so fallback behavior is deterministic. OmniRoute's OpenCode-compatible provider (`home/dot_config/opencode/opencode.json`) only accepts model IDs pre-declared in its `models` map — unlike calling the OmniRoute HTTP API directly, unregistered IDs fail with `ProviderModelNotFoundError` even though OmniRoute itself serves them. `opencode-zen/deepseek-v4-flash-free` is registered there; add any other `omniroute/<provider>/<model>` reference used in `omo.jsonc` to that same map first.
- **Never reference the native `opencode/*` provider**: always go through `omniroute/opencode-zen/...` (or another explicit provider) instead of bare `opencode/...` model IDs in `omo.jsonc`. The native path surfaces OpenCode Zen's raw upsell text ("Free usage exceeded, subscribe to Go") once the shared Zen account quota is exhausted, while `omniroute/opencode-zen/...` rewrites the same failure to a generic 429 — same root cause either way, so there's no reliability upside to the native path, only noisier errors. `omniroute/opencode-go/deepseek-v4-flash` was tried twice as a paid-plan escape hatch positioned right after the zen-free primary, and dropped both times: both the free Zen and paid Go deepseek routes hit the same shared "Structurally heavy chat request capacity is busy" / "Chat admission capacity" infra errors under load, observed live hammering the go hop multiple times per second across parallel explore subagents — so it adds retry noise without a reliability gain. Every zen-primary chain now falls straight from primary to openai instead.
- **OMO resets fallback memory every turn**: oh-my-openagent's `runtime-fallback` hook keys its fallback state (`fallbackIndex`, `failedModels` cooldown map) per session, but each new subagent turn re-requests the agent's configured primary model — since that almost never matches whatever model the previous turn's fallback had landed on, the hook treats it as a manual model change and calls `createFallbackState`, wiping the cooldown memory. So a chain can never "remember" that its first N entries just failed seconds ago; it re-walks the chain from the top on every turn. There is no config flag for this — the only mitigation is keeping known-unreliable models (dead free tiers, exhausted quotas) out of the front of each chain, since every turn pays the cost of walking past whatever sits there.
- **Qwen/bailian provider removed**: the `bailian-token-plan-personal` (QwenCloud) provider was dropped from `opencode.json` and all `bailian-token-plan-personal/qwen*` entries were stripped from every agent's `fallback_models` and category's `models` chain in `omo.jsonc` after its weekly token-plan quota repeatedly exhausted outright (not just rate-limited) with a fixed weekly reset. Do not reintroduce Qwen/bailian model IDs into either config.
