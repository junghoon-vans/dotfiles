...
- **OmniRoute persistence**: `./setup.sh opencode` loads `home/Library/LaunchAgents/com.dotfiles.omniroute.plist` (label `com.dotfiles.omniroute`) via `launchctl bootstrap`/`enable` so `omniroute serve` stays up across logins and restarts on crash (`RunAtLoad`/`KeepAlive`), matching the existing `com.dotfiles.weekly-disk-maintenance` LaunchAgent pattern. `oh-my-openagent.json` routes through `omniroute/auto/best-free` as the fallback immediately after `opencode/deepseek-v4-flash-free` in every chain that uses it, since OmniRoute auto-selects among live free upstream models instead of pinning to one that may be rate-limited.
