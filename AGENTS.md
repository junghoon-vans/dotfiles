...
- **Direct OpenCode Zen free fallback**: `home/dot_omo/omo.jsonc` adds the direct Zen free `opencode/mimo-v2.5-free` alongside every `omniroute/opencode-zen/*-free` primary entry in the fallback chains, so free-tier routing also covers OmniRoute outages. Keep the reliable bailian entry FIRST in free-primary agent chains: OMO resets fallback state when a failed subagent turn is cancelled, so the chain effectively re-tries its first reachable entry — a failing free model in front spins the session without ever reaching paid models. When Zen's shared free quota is exhausted (429 `FreeUsageLimitError` with a long `retry-after`), `runtime_fallback` (`retry_on_errors` includes 402/429/5xx) falls through to bailian/openai paid models instead of hanging on OpenCode core's retry-after delay showing "Free usage exceeded, subscribe to Go". OmniRoute rewrites Zen errors to generic 429s (no `FreeUsageLimitError` body), while direct `opencode/*` calls surface the upsell message — both paths fall through the same chain. `omniroute/opencode-go/deepseek-v4-flash` was tried as a paid-plan fallback but dropped: both the free Zen and paid Go deepseek routes hit the same shared "Structurally heavy chat request capacity is busy" / "Chat admission capacity" infra errors under load, so Go added retry noise without a reliability gain — every chain now falls straight from bailian to `openai/gpt-5.6-luna` (`max` reasoning) instead.
