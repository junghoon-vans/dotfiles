...
- **Playwright MCP for Codex**: `./setup.sh codex` and `./setup.sh codex-mcp` register the Codex `playwright` MCP server through `npx -y @playwright/mcp@latest` and point it at Aside with `PLAYWRIGHT_MCP_EXECUTABLE_PATH`. Aside is installed from the Brewfile cask; override `ASIDE_BROWSER_EXECUTABLE` if the app lives elsewhere.
