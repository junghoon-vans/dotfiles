...
- **OmniRoute persistence**: `./setup.sh opencode` loads `home/Library/LaunchAgents/com.dotfiles.omniroute.plist` (label `com.dotfiles.omniroute`) via `launchctl bootstrap`/`enable` so `omniroute serve` stays up across logins and restarts on crash (`RunAtLoad`/`KeepAlive`), matching the existing `com.dotfiles.weekly-disk-maintenance` LaunchAgent pattern. The unified OMO config at `home/dot_omo/omo.jsonc` pins free-tier routing to `omniroute/opencode-zen/deepseek-v4-flash-free` instead of `omniroute/auto/*` (which rotated among unreliable free upstreams), so fallback behavior is deterministic. OmniRoute's OpenCode-compatible provider (`home/dot_config/opencode/opencode.json`) only accepts model IDs pre-declared in its `models` map — unlike calling the OmniRoute HTTP API directly, unregistered IDs fail with `ProviderModelNotFoundError` even though OmniRoute itself serves them. `opencode-zen/deepseek-v4-flash-free` and `opencode-go/deepseek-v4-flash` are registered there; add any other `omniroute/<provider>/<model>` reference used in `omo.jsonc` to that same map first.
- **Never reference the native `opencode/*` provider**: always go through `omniroute/opencode-zen/...` or `omniroute/opencode-go/...` (or another explicit provider) instead of bare `opencode/...` model IDs in `omo.jsonc`. The native path surfaces OpenCode Zen's raw upsell text ("Free usage exceeded, subscribe to Go") once the shared Zen account quota is exhausted, while the `omniroute/...` alias rewrites the same failure to a generic 429 — same root cause either way, so there's no reliability upside to the native path, only noisier errors. Every chain whose primary is `omniroute/opencode-zen/deepseek-v4-flash-free` tries `omniroute/opencode-go/deepseek-v4-flash` (paid Go plan) as its first fallback before bailian/openai — both the free Zen and paid Go deepseek routes can hit the same shared "Structurally heavy chat request capacity is busy" / "Chat admission capacity" infra errors under load, so this hop isn't always a reliability win, but it's cheap to try before falling through to paid bailian/openai.
