...
- `setup.sh` flow is `bootstrap → brew-packages → languages → blockchain → links → apps → omp → opencode → opencode-skills → codex → codex-agents → codex-skills → karabiner → macos-shortcuts → maintenance → macos`.
...
./setup.sh languages omp opencode  # Run specific default commands
...
- **Oh My Pi**: `./setup.sh omp` installs `@oh-my-pi/pi-coding-agent` globally through Bun and verifies the resulting `omp` command. Requires Bun runtime.
- **OmniRoute persistence**: `./setup.sh opencode` loads `home/Library/LaunchAgents/com.dotfiles.omniroute.plist` (label `com.dotfiles.omniroute`) via `launchctl bootstrap`/`enable` so `omniroute serve` stays up across logins and restarts on crash (`RunAtLoad`/`KeepAlive`), matching the existing `com.dotfiles.weekly-disk-maintenance` LaunchAgent pattern. The unified OMO config at `home/dot_omo/omo.jsonc` pins paid DeepSeek routing to `omniroute/opencode-go/deepseek-v4-flash` instead of free-tier or `omniroute/auto/*` routes, so fallback behavior is deterministic. OmniRoute's OpenCode-compatible provider (`home/dot_config/opencode/opencode.json`) only accepts model IDs pre-declared in its `models` map — unlike calling the OmniRoute HTTP API directly, unregistered IDs fail with `ProviderModelNotFoundError` even though OmniRoute itself serves them. The OMO references `opencode-go/deepseek-v4-flash`, `gemini/gemini-3-flash-preview`, and `kiro/claude-sonnet-4.5`; each is registered there. Add any other `omniroute/<provider>/<model>` reference used in `omo.jsonc` to that same map first.
- **Never reference the native `opencode/*` provider**: always go through `omniroute/<provider>/<model>` (or another explicit provider) instead of bare `opencode/*` model IDs in `omo.jsonc`. The `opencode-zen/deepseek-v4-flash-free` and `oc/deepseek-v4-flash-free` routes were removed after live requests began returning HTTP 401 with "Free promotion has ended". The paid `omniroute/opencode-go/deepseek-v4-flash` route is the verified replacement; its live request returned HTTP 200. The `opencode-zen/deepseek-v4-flash` route remains unused because the current gateway returned HTTP 401 for a missing API key.
